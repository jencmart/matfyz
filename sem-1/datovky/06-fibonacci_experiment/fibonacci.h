...
/* Decrease priority of node to new_prio.
    ...
    */
    ...
    int calculate_trees(){
        Node * first = meta_root.first_child;
        ...
        int cnt = 1;
        while(pointer->next_sibling != first){
...
protected:
    ...
    void consolidate() {
        std::vector<Node*> buckets(max_rank()*2, nullptr); // +5
        while (meta_root.first_child) {
            Node *node = remove(meta_root.first_child);
            while (Node *&b = buckets.at(node->rank)) { // log n
    ...
    virtual void add_child(Node* parent, Node *node) {
        ...
        // unmark all nodes that will be root nodes -- todo -- this is new
        if (parent == &meta_root && node->marked)
        {
            node->marked = false;
            colored_counter -= 1;
        }
