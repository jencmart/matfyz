...
# reward ... table for the prisoner dilemma
def reward(m1, m2):
...
def play_game(s1, s2, iters, verbose=False):
    s1.reset()  # at the beginning reset the strategy
    s2.reset()
    if verbose:
    ...
    for _ in range(iters):
        ...
        # inform both player how was opponents last move
        s1.last_move(m1, m2)
        s2.last_move(m2, m1)
...
if __name__ == '__main__':
    ...
    # Import strategies as the python modules
    ...
    # each strategy is created by create_strategy() method
