...
class Tree {
  public:
    ...
    virtual void doubleRotate(Node * node){
        Node* x = node;
        Node* y = node->parent;
    ...
    virtual Node* lookup(int key) {
        ...
        while (node) {
            ...
            // go left
            else if (key < node->key){ // todo - new zmena
    ...
    float caclRankRec(Node * node, float & totalPotential){
        float thisRank = 1.0f;

        if(node->left)
            thisRank += caclRankRec(node->left, totalPotential);

        if(node->right)
            thisRank += caclRankRec(node->right, totalPotential);

        totalPotential += (float) (std::log(thisRank) / log(2.0));
        return thisRank;
    }

    float calcPotential(){
        float totalPotential = 0;
        caclRankRec(root, totalPotential);
